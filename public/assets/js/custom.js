...
(function($){	
	...
	$(document).ready(function(){		
		...
		/* Page Loader */
		$window.load(function(){$(".page-loader").fadeOut("slow");});
		...
		/*Paroller Background*/
		if($('.paroller-bg').length){$('.paroller-bg').paroller();}
		/*Paroller Foreground*/
		if($('.paroller-fore').length){$('.paroller-fore').paroller();}
		...
		if(css_out != ''){$('head').append('<style id="theme-dynamic-styles">'+ css_out +'</style>');}
		/* Full search toggle */
	...
	$window.on('load', function(){ 
		...
		if(o_stat){		
			...
			$window.on('scroll', function(){
				...
				$('section[id*="section-"], body').each(function (){
					...
					var	targetOffset = cur_ele.offset().top - scl_offst;
		...
		if($(".footer-hidden")){
			zoacresFooterHidden();
			$window.smartresize(function(){zoacresFooterHidden();});	
		}
		...
		//Google map init
		if($("#site-google-map").length){ initniskarGmap();}
	}); // window load end
	...
	function initniskarGmap(){
		...
		var LatLng = {lat: -7.278065, lng: 112.7734039};
